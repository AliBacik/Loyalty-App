...
export const action = async ({ request }) => {
  ...
  try {
    const { customerId, shop, gender, email } = await request.json();

    if ((!customerId && !email) || !shop || gender === undefined) {
      return new Response(JSON.stringify({ error: "Missing required fields: (customerId or email), shop, gender" }), { status: 400, headers: corsHeaders });
    }
    ...
    // 2. Set customer metafield for gender in namespace `loyalty_program`
    const mutation = `
    ...
    if (updateJson.errors || updateJson.data?.customerUpdate?.userErrors?.length > 0) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to set gender metafield', details: updateJson.errors || updateJson.data?.customerUpdate?.userErrors }), { status: 400, headers: corsHeaders });
    }

    return new Response(JSON.stringify({ success: true, message: 'Gender updated', value: gender }), { status: 200, headers: corsHeaders });
  } catch (err) {
    console.error('Error in setgender:', err);
